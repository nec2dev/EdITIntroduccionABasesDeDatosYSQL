...
SELECT SEXO, FECHA, TIPO_PARTO, LOCAL_PART, SUBSTRING(fecha, 4,2) AS MES FROM nacimientos ORDER BY MES;
...
SELECT COUNT(IDPEDIDO) FROM PEDIDOS_NEPTUNO;
SELECT * FROM PEDIDOS_NEPTUNO;
...
SELECT EMPLEADO, ROUND(SUM(CARGO), 2) AS FACTURACION, ROUND(AVG(CARGO), 2) AS PROMEDIO, MAX(CARGO) 
AS 'MEJOR VENTA', MIN(CARGO) AS 'PEOR VENTA', COUNT(CARGO) AS VENTAS FROM PEDIDOS_NEPTUNO 
GROUP BY EMPLEADO;